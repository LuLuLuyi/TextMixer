...
def train_inversion_model(config, tokenizer, model, train_dataloader, eval_dataloader,test_dataloader, use_wandb=True, output_dir=None):
    ...
    test_dataloader = dataloader2memory(test_dataloader, model, config.target_layer, device)
    print('done')
...
def train_mlc_model(config, tokenizer, model, train_dataloader, eval_dataloader,test_dataloader, use_wandb=True, output_dir=None, inversion_epochs=10, inversion_lr=5e-5):
    device ='cuda'
...
def main():
    ...
    model, adversarial_model, optimizer, adversarial_model_optimizer, train_dataloader, eval_dataloader, test_dataloader, lr_scheduler, adversarial_model_lr_scheduler = accelerator.prepare(
        model, adversarial_model, optimizer, adversarial_model_optimizer, train_dataloader, eval_dataloader, test_dataloader, lr_scheduler, adversarial_model_lr_scheduler
    )
    ...
    # # test!
    # eval_metric = evaluate_with_knn_attack(model, test_dataloader, metric, accelerator, tokenizer=tokenizer, label_list=label_list, target_layer=args.target_layer, topk=knn_topk)
    # if args.use_wandb:
    ...
    # train mlc model
    model_attack_acc = train_mlc_model(config, tokenizer, model, train_dataloader, eval_dataloader, test_dataloader, use_wandb=args.use_wandb, output_dir=args.output_dir, inversion_epochs=10, inversion_lr=args.inversion_lr)
