...
def test_with_knn_attack(model, dataloader, output_dir, nullification_rate=-1 ,topk=5, target_layer=3):
    # device = 'cuda'
...
if __name__ == "__main__":
    ...
    topk = 1
    # set hyparam
    model_path = '/root/contrastive_privacy/version_noise/ckpts/sst2/epsilon0.5_nu0.1_lr1e-5'
    epsilon = 0.5
    nullification_rate = 0.1 
    wandb_name = f'best_test_noise_sst2_epsilon0.5_nu0.1'
    ...
    print('################# do test #################')
    eval_metric = test_with_knn_attack(model, test_dataloader, output_dir=model_path, nullification_rate=nullification_rate, topk=10, target_layer=target_layer)
    ...
    train_dataloader = dataloader2memory_with_word_dropout(train_dataloader, model, nullification_rate, target_layer)
    eval_dataloader = dataloader2memory_with_word_dropout(eval_dataloader, model, nullification_rate, target_layer)
    test_dataloader = dataloader2memory_with_word_dropout(test_dataloader, model, nullification_rate, target_layer)
    print('done')
