...
def train_inversion_model(config, tokenizer, model, train_dataloader, eval_dataloader, use_wandb=True, output_dir=None):
    ...
    train_dataloader = dataloader2memory_with_word_dropout(train_dataloader, model, config.target_layer, device)
    eval_dataloader = dataloader2memory_with_word_dropout(eval_dataloader, model, config.target_layer, device)
    print('done')
...
def main():
    ...
    if args.use_wandb:
        project_name = f'finetune_{args.dataset_name}' if 'ontonotes' not in args.dataset_name else 'finetune_ontonotes'
    ...
    if args.train_task_model:
        ...
        for epoch in range(starting_epoch, args.num_train_epochs):
            ...
            # save the best model
            if eval_metric['f1'] > best_task_f1:
