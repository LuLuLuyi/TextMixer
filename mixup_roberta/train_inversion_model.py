...
#             hit_cnt = 0
#             total_cnt = 0
#             with open(f'/root/mixup/mixup_roberta/case_study/datamux-sst2-10/inversion_epoch{epoch}.txt','w') as f:
#                 for batch in eval_dataloader:
#                     batch = {key:value.to(device) for key,value in batch.items()}
#                     # batch['output_hidden_states'] = True
...
def train_inversion_model(config, tokenizer, model, train_dataloader, eval_dataloader, use_wandb=True):
    ...
    with open(f'/root/mixup/mixup_roberta/case_study/{case_study_dir}/mux_tokens_stat.txt','w') as f:
        for mux_tokens in mux_tokens_list:
            for mux_token in mux_tokens:
                f.write(f"{mux_token:<15s}")
            f.write('\n')
    with open(f'/root/mixup/mixup_roberta/case_study/{case_study_dir}/mux_tokens_conbine_stat.txt','w') as f:
        mux_conbination = {}
        for mux_tokens in mux_tokens_list:
...
def parse_args():
    ...
    parser.add_argument("--output_dir", type=str, default='/root/mixup/mixup_roberta/ckpts', help="Where to store the final model.")
...
def main():
    ...
    model = RobertaSequenceClassificationMuxed.from_pretrained(
